...
import {
  Flame, Footprints, Trophy, CalendarDays, ChevronLeft, ChevronRight,
  BarChart2, PlusCircle, Droplets, Moon, Plus, Minus, X, Edit3
} from 'lucide-react';
import { getProfile, getWorkouts, getStreakInfo, recalculateStreakFull, getDailyStats, saveDailyStats, getAllDailyStats } from '../services/storage';
...
const Dashboard = () => {
  ...
  const [tempHealthStats, setTempHealthStats] = useState({ water: 0, sleep: 0 });
  ...
  const PIE_COLORS = [COLORS.chart1, COLORS.chart2, COLORS.chart3, COLORS.chart4, COLORS.chart5, '#8884d8', '#82ca9d'];
    ...
    const loadData = async () => {
      try {
        ...
        if (loadedWorkouts.length > 0) {
          const workoutYears = new Set(loadedWorkouts.map(w => new Date(w.date).getFullYear()));
  ...
  const openHealthModal = () => {
    setTempHealthStats({ water: todayStats.waterIntake, sleep: todayStats.sleepHours });
  ...
  const todaySteps = todayWorkouts.reduce((acc, w) => acc + (Number(w.steps) || 0), 0);
  const todayCalories = todayWorkouts.reduce((acc, w) => acc + (Number(w.caloriesBurned) || 0), 0);
  ...
  const weeklyData = getLast7Days().map(date => {
    ...
    return {
      ...
      calories: dayWorkouts.reduce((acc, w) => acc + (Number(w.caloriesBurned) || 0), 0),
  ...
  const intensityCount = workouts.reduce((acc, w) => {
    if (w.intensity) {
  ...
  const intensityData = [
    ...
    { name: 'Medium', value: intensityCount[Intensity.Medium], color: COLORS.accent },
    { name: 'High', value: intensityCount[Intensity.High], color: COLORS.danger },
  ].filter(d => d.value > 0);

  // 3. Top Calorie Burning Activities
  const caloriesByType = Object.entries(workouts.reduce((acc, w) => {
    if (w.type) {
  ...
  heatmapWeeks.forEach((week, index) => {
    const containsFirstOfMonth = week.some(d => new Date(d.date + 'T12:00:00').getDate() === 1);
    if (containsFirstOfMonth) {
      const monthName = week.find(d => new Date(d.date + 'T12:00:00').getDate() === 1)?.month;
      if (monthName && (monthLabels.length === 0 || monthLabels[monthLabels.length - 1].label !== monthName)) {
  ...
  const getIntensityClass = (count, inSelectedYear) => {
    if (!inSelectedYear) return 'bg-transparent border-dashed border-border opacity-20';
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-popover border border-border p-2 rounded shadow-lg text-sm z-50">
          ...
          <p className="text-primary font-mono">{`${payload[0].name}: ${Number(payload[0].value).toLocaleString()}`}</p>
        </div>
      );
    }
    return null;
  ...
  const ChartEmptyState = ({ title, message }) => (
    <div className="h-full w-full flex flex-col items-center justify-center text-center p-6 bg-gray-50 dark:bg-zinc-900 rounded-lg border-2 border-dashed border-gray-200 dark:border-zinc-700">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 max-w-[200px]">{message}</p>
      <button
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-serif font-bold tracking-tight text-foreground">Hello, {profile.name.split(' ')[0]}</h1>
          <p className="text-muted-foreground">Your fitness analytics at a glance.</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-accent/50 px-4 py-2 rounded-full border border-border">
            ...
            <span className="font-mono font-bold">{streak?.totalPoints} pts</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2 relative overflow-hidden group">
          <div className="flex justify-between items-start">
            <span className="text-muted-foreground text-sm font-medium">Sleep</span>
            <div className="p-2 bg-indigo-500/10 rounded-lg text-indigo-500"><Moon size={20} /></div>
          </div>
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-bold font-mono">{todayStats.sleepHours}</span>
          ...
          <div className="w-full bg-secondary h-2 rounded-full overflow-hidden mt-3">
            <div
              ...
              style={{ width: `${Math.min(100, (todayStats.sleepHours / 8) * 100)}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
          <div className="flex justify-between items-start mb-2">
            <span className="text-muted-foreground text-sm font-medium">Water</span>
            <div className="p-2 bg-blue-500/10 rounded-lg text-blue-500"><Droplets size={20} /></div>
          </div>
          <div className="flex justify-between items-center px-1">
            {Array.from({ length: 8 }).map((_, i) => (
              <button
                key={i}
                onClick={() => handleWaterUpdate(i + 1 === todayStats.waterIntake && todayStats.waterIntake === 1 ? 0 : i + 1)}
                className={`transition-all duration-300 hover:scale-110 ${i < todayStats.waterIntake ? 'text-blue-500' : 'text-secondary dark:text-zinc-800'}`}
                title="Toggle glass"
              >
                <svg
          ...
          <div className="text-xs text-muted-foreground text-center mt-2 flex justify-between items-center px-1">
            ...
            <button onClick={() => handleWaterUpdate(todayStats.waterIntake + 1)} className="text-blue-500 hover:text-blue-600 text-[10px] font-bold uppercase">+ Add</button>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-serif font-bold tracking-tight">Activity Log</h3>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 bg-secondary rounded-lg p-0.5">
              ...
              <span className="text-xs font-bold font-mono px-2 min-w-[40px] text-center">{selectedYear}</span>
              <button
                onClick={() => setSelectedYear(prev => prev + 1)}
                className="p-1 hover:bg-background rounded-md text-muted-foreground transition-colors disabled:opacity-30"
        ...
        <div className="w-full overflow-x-auto pb-2 no-scrollbar">
          <div className="min-w-max">
            <div className="flex mb-2 pl-8">
              {monthLabels.map((m, i) => (
                <span key={i} className="text-xs text-muted-foreground" style={{ width: `${(monthLabels[i + 1]?.index - m.index) * 16 || 40}px`, minWidth: '40px', display: 'inline-block' }}>
                  {m.label}
                </span>
              ))}
            </div>
            <div className="flex gap-1">
              <div className="flex flex-col gap-1 pr-2 pt-[14px]">
                ...
                <span className="text-[10px] h-3 text-muted-foreground leading-3">Mon</span>
                <span className="text-[10px] h-3 text-muted-foreground leading-3"></span>
                <span className="text-[10px] h-3 text-muted-foreground leading-3">Wed</span>
                <span className="text-[10px] h-3 text-muted-foreground leading-3"></span>
                <span className="text-[10px] h-3 text-muted-foreground leading-3">Fri</span>
                <span className="text-[10px] h-3 text-muted-foreground leading-3"></span>
              </div>
              {heatmapWeeks.map((week, weekIndex) => (
                <div key={weekIndex} className="flex flex-col gap-1">
                  {week.map((day) => (
                    <div
                      key={day.date}
                      className={`w-3 h-3 rounded-[2px] ${getIntensityClass(day.count, day.inSelectedYear)} border border-transparent hover:border-black/10 dark:hover:border-white/20 transition-all cursor-pointer`}
                      title={`${day.count} workouts on ${new Date(day.date + 'T12:00:00').toLocaleDateString()}`}
                    />
                  ))}
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-6">Weekly Steps</h3>
          <div className="h-64 flex-1">
            {hasWeeklySteps ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.2} stroke={COLORS.grid} />
                    <XAxis dataKey="day" tick={{ fill: COLORS.text }} axisLine={false} tickLine={false} />
                    <YAxis tick={{ fill: COLORS.text }} axisLine={false} tickLine={false} />
                    <Tooltip cursor={{ fill: 'var(--accent)' }} content={<CustomTooltip />} />
                    <Bar dataKey="steps" fill={COLORS.chart1} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-6">Weekly Calories</h3>
          <div className="h-64 flex-1">
            {hasWeeklyCalories ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={weeklyData}>
                    ...
                    <Line type="monotone" dataKey="calories" stroke={COLORS.chart2} strokeWidth={3} dot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-6">Sleep Trends</h3>
          <div className="h-64 flex-1">
            {hasWeeklySleep ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={weeklyData}>
                    ...
                    <YAxis tick={{ fill: COLORS.text }} axisLine={false} tickLine={false} />
                    <Tooltip content={<CustomTooltip />} />
                    <Area type="monotone" dataKey="sleep" stroke={COLORS.chart3} fill={COLORS.chart3} fillOpacity={0.2} strokeWidth={3} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-6">Hydration</h3>
          <div className="h-64 flex-1">
            {hasWeeklyWater ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData}>
                    ...
                    <YAxis tick={{ fill: COLORS.text }} axisLine={false} tickLine={false} />
                    <Tooltip cursor={{ fill: 'var(--accent)' }} content={<CustomTooltip />} />
                    <Bar dataKey="water" fill={COLORS.secondary} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-2">Activity Types</h3>
          <p className="text-sm text-muted-foreground mb-4">Breakdown by workout type</p>
          <div className="h-64 flex-1">
            {pieData.length > 0 ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-2">Intensity Levels</h3>
          <p className="text-sm text-muted-foreground mb-4">Training effort distribution</p>
          <div className="h-64 flex-1">
        ...
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col lg:col-span-1 md:col-span-2">
          <h3 className="text-lg font-serif font-bold tracking-tight mb-2">Top Burners</h3>
          <p className="text-sm text-muted-foreground mb-4">Most effective activities</p>
          <div className="h-64 flex-1">
            {caloriesByType.length > 0 ? (
              <div style={{ width: '100%', height: '100%', minHeight: '200px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart layout="vertical" data={caloriesByType} margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} opacity={0.2} stroke={COLORS.grid} />
                    <XAxis type="number" hide />
                    <YAxis dataKey="name" type="category" width={70} tick={{ fill: COLORS.text, fontSize: 12 }} axisLine={false} tickLine={false} />
                    <Tooltip cursor={{ fill: 'var(--accent)' }} content={<CustomTooltip />} />
                    <Bar dataKey="value" fill={COLORS.chart4} radius={[0, 4, 4, 0]} barSize={20} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
            ...
            <form onSubmit={saveHealthStats} className="space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setTempHealthStats(prev => ({ ...prev, sleep: Number(e.target.value) }))}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium flex items-center gap-2">
                  <Droplets size={16} className="text-blue-500" /> Water Intake (Glasses)
                </label>
                <input
                  type="number"
                  step="1"
                  ...
                  onChange={(e) => setTempHealthStats(prev => ({ ...prev, water: Number(e.target.value) }))}
                  className="w-full bg-background border border-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:outline-none"
                />
              </div>
              <button
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-2">
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-2xl font-bold font-mono text-foreground">{val.toLocaleString()}</span>
        <span className="text-xs text-muted-foreground">/ {g.toLocaleString()}</span>
      </div>
