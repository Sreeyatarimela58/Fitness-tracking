...
const Layout = ({ children }) => {
  ...
  if (location.pathname === '/') {
    return <main className="min-h-screen bg-background text-foreground transition-colors duration-300 font-sans">{children}</main>;
  }
      ...
      <nav className="fixed top-4 md:top-6 inset-x-4 md:inset-x-0 md:w-full md:max-w-4xl md:mx-auto z-50 animate-in fade-in slide-in-from-top-4 duration-700">
        <div className="h-16 md:h-20 px-4 md:px-6 rounded-2xl border border-border/60 bg-background/80 backdrop-blur-xl shadow-lg ring-1 ring-black/5 dark:ring-white/5 flex items-center justify-between">
          ...
          <div className="flex-shrink-0">
            <Link to="/dashboard" className="group flex items-center gap-2 md:gap-3">
              <AnimatedLogo />
              <div className="hidden md:flex flex-col gap-0 leading-none">
                <span className="font-serif font-bold tracking-tight text-xl transition-colors duration-300 group-hover:text-primary">FitTrack</span>
              </div>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-1 md:gap-2">
            {navItems.map((item) => (
              <NavLink
                ...
                to={item.path}
                className={({ isActive }) =>
                  `relative flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all duration-300 hover:scale-105 active:scale-95 ${isActive
          ...
          <div className="flex items-center gap-2 md:gap-4 pl-2 border-l border-border/50">
            ...
            <NavLink to="/profile" className={({ isActive }) => `group flex items-center gap-2 md:gap-3 p-1 pr-3 rounded-full transition-all border hover:scale-105 active:scale-95 ${isActive ? 'bg-secondary border-border' : 'hover:bg-secondary border-transparent'}`}>
              ...
              <div className="h-8 w-8 rounded-full bg-secondary border border-border text-foreground flex items-center justify-center text-xs font-bold shadow-sm group-hover:border-primary/50 transition-colors">
                {profile ? profile.name.charAt(0).toUpperCase() : <User size={14} />}
              </div>
